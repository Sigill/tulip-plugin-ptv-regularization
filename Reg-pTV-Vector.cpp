...
bool Reg_pTV_Vector::check(std::string &err)
{
	try {
		if(dataSet == NULL)
		...
		// TODO Check f0_size is constant over graph
		this->f0_size = this->f0->getNodeValue(graph->getOneNode()).size();
...
//======================================================
bool Reg_pTV_Vector::run() {
	DoubleVectorProperty *tmp, *fn = NULL, *fnp1 = NULL, *grad = NULL;
	Iterator<node> *itNodesU;
	Iterator<edge> *itEdges;
	node u, v;
	edge e;
	double b, lambda_u, w_e;
	std::vector< double > num(this->f0_size), denum(this->f0_size), grad_u(this->f0_size), grad_v(this->f0_size), fn_u, fn_v, f0_u;
	...
	fn = graph->getLocalProperty< DoubleVectorProperty >(fn_name);
	...
	for(unsigned int i = 0; i < iter_max; ++i) {
		if(this->is_anisotropic && !this->is_isotropic) {
			...
			while(itNodesU->hasNext()) {
				...
				f0_u = f0->getNodeValue(u);

				lambda_u = this->wl->getNodeValue(u);
				for(j = 0; j < this->f0_size; ++j) {
					num[j] = lambda_u * f0_u[j];
					denum[j] = lambda_u;
				}
				...
				while(itEdges->hasNext()) {
					...
					w_e = this->wl->getEdgeValue(e);

					for(j = 0; j < this->f0_size; ++j) {
						/*
						 * fabs() is regularized to avoid 0
						 */
						b = pow(w_e, this->p / 2) * pow(fabs(fn_u[j] - fn_v[j]) + this->epsilon, this->p - 2);
						num[j] += 2 * b * fn_v[j];
						denum[j] += 2 * b;
					}
				}
			...
			while(itNodesU->hasNext()) {
				...
				while(itEdges->hasNext()) {
					...
					for(j = 0; j < this->f0_size; ++j)
						grad_u[j] += w_e * pow(fabs(fn_u[j] - fn_v[j]), 2);
				}
				delete itEdges;
				 ...
				 */
				for(j = 0; j < this->f0_size; ++j)
					grad_u[j] = pow(grad_u[j] + epsilon_squared, gradient_power);

				grad->setNodeValue(u, grad_u); 
			}
			delete itNodesU;
			...
			while(itNodesU->hasNext()) {
				...
				while(itEdges->hasNext()) {
					...
					for(j = 0; j < this->f0_size; ++j) {
						b = this->wl->getEdgeValue(e) * (grad_u[j] + grad_v[j]);
						num[j] += b * fn_v[j];
						denum[j] += b;
					}
				}
...
void Reg_pTV_Vector::exportGraph(const int i) {
